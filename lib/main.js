...
var apiBase = 'https://www.googleapis.com/language/translate/v2/',
    maxPostQueryLen = 5000, // Limit is actually 2000, but let's just be safe //modified from 1600 to 5000 for maxPostQuery Length
...
//modified GET to POST to allow for max character length limit
var postRequestWithApi = function(apiKey) {
  return function(path, data, done) {
    var url = apiBase,
        formData = _.extend({key: apiKey}, data),
...
var splitArraysForGoogle = function(arr, result) {
  if (arr.join(',').length > maxPostQueryLen && arr.length !== 1) {
...
module.exports = function(apiKey, newConcurrentLimit) {
  ...
  api.translate = function(strings, sourceLang, targetLang, done) {
    ...
    if (Array.isArray(strings) && strings.join(',').length > maxPostQueryLen && strings.length !== 1) {
      stringSets = [];
